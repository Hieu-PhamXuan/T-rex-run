...
void close(SDL_Window* window, SDL_Renderer* renderer, SDL_Texture* background, SDL_Texture* sonicJumpFrames[], SDL_Texture* sonicRun[], SDL_Texture* obstacles[]) {
...
int main(int argc, char* argv[]) {
    ...
    if (!background) {
        close(window, renderer, background, nullptr, nullptr, nullptr);
        return -1;
    }
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (!sonicJumpFrames[i]) {
            close(window, renderer, background, sonicJumpFrames, nullptr, nullptr);
            return -1;
        }
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (!sonicRun[i]) {
            close(window, renderer, background, sonicJumpFrames, sonicRun, nullptr);
            return -1;
        }
    }
    ...
    SDL_Rect obstaclePos = {SCREEN_WIDTH, GROUND_Y + 30, 60, 60};
    ...
    close(window, renderer, background, sonicJumpFrames, sonicRun, obstacles);
