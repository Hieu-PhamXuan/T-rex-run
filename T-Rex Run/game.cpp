...
void Game::loadAssets() {
    ...
    for (int i = 0; i < 3; ++i) {
        string path = "C:\\T-Rex Run\\Sonic\\cloud" + to_string(i + 1) + ".png";
    ...
    for (int i = 0; i < 8; i++) {
        string path = "C:\\T-Rex Run\\Sonic\\bird_white_" + to_string(i + 1) + ".png";
        birdFrames[i] = loadTexture(path.c_str(), renderer);
    }
    for (int i = 0; i < 8; i++) {
        string path = "C:\\T-Rex Run\\Sonic\\bird_black_" + to_string(i + 1) + ".png";
...
void Game::update() {
    if (!isGameOver) {
        ...
        if (obstaclePos.x + obstaclePos.w < 0) {
            obstaclePos.x = SCREEN_WIDTH + 100;
